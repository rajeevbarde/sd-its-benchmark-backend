...
#[async_trait]
impl BulkRepository<GpuBase, i64> for GpuBaseRepository {
    async fn bulk_create(&self, entities: Vec<GpuBase>) -> Result<Vec<GpuBase>, Error> {
    ...
    async fn bulk_update(&self, entities: Vec<GpuBase>) -> Result<Vec<GpuBase>, Error> {
        if entities.is_empty() {
            return Ok(vec![]);
        }
...
#[async_trait]
impl<'a> BulkTransactionRepository<'a, GpuBase, i64> for GpuBaseRepository {
    async fn bulk_create_tx(&self, entities: Vec<GpuBase>, tx: &mut Transaction<'a, Sqlite>) -> Result<Vec<GpuBase>, Error> {
        if entities.is_empty() {
            return Ok(vec![]);
        }
    ...
    async fn bulk_update_tx(&self, entities: Vec<GpuBase>, tx: &mut Transaction<'a, Sqlite>) -> Result<Vec<GpuBase>, Error> {
        if entities.is_empty() {
            return Ok(vec![]);
        }
    ...
    async fn delete_all_tx(&self, tx: &mut Transaction<'a, Sqlite>) -> Result<usize, Error> {
        let result = sqlx::query!("DELETE FROM GPUBase")
            .execute(&mut **tx)
            .await?;
