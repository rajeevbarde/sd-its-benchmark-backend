...
#[async_trait]
impl BulkRepository<SystemInfo, i64> for SystemInfoRepository {
    async fn bulk_create(&self, entities: Vec<SystemInfo>) -> Result<Vec<SystemInfo>, Error> {
    ...
    async fn bulk_update(&self, entities: Vec<SystemInfo>) -> Result<Vec<SystemInfo>, Error> {
        if entities.is_empty() {
            return Ok(vec![]);
        }
...
#[async_trait]
impl<'a> BulkTransactionRepository<'a, SystemInfo, i64> for SystemInfoRepository {
    async fn bulk_create_tx(&self, entities: Vec<SystemInfo>, tx: &mut Transaction<'a, Sqlite>) -> Result<Vec<SystemInfo>, Error> {
        if entities.is_empty() {
            return Ok(vec![]);
        }
    ...
    async fn bulk_update_tx(&self, entities: Vec<SystemInfo>, tx: &mut Transaction<'a, Sqlite>) -> Result<Vec<SystemInfo>, Error> {
        if entities.is_empty() {
            return Ok(vec![]);
        }
    ...
    async fn delete_all_tx(&self, tx: &mut Transaction<'a, Sqlite>) -> Result<usize, Error> {
        let result = sqlx::query!("DELETE FROM SystemInfo")
            .execute(&mut **tx)
            .await?;
